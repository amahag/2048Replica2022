...
public class GUI2048 extends Application 
{
	...
	static GenerateSquares[][] boardRotated = new GenerateSquares[4][4];// Creates a 4x4 array, which is the game board that rotates.
	...
	static boolean keepPlayingTheGame = false;							// This variable prevents from duplication code that was needed in my original board.
																		// It is used to prevent the game from popping up the congratulations window everytime 
																		// a 2048 tile is on the board.
	
	static File saveGame = new File("2048.dat");						// Creates a file to save the game.
	static Label score = new Label();									// Made static so that once you load the old game, loadTheLastGame() can see this label
																		// and can place the old score at the top of the game.
	
	static Label count = new Label();									// Made static so that once you load the old game, loadTheLastGame() can see this label
																		// and can place the old count at the top of the game.
	
	// The start method creates the 2048 GUI display.  The stage consists of BorderPanes, HBoxes, and StackPanes.
    ...
    public void start(Stage primaryStage) 
    {
    	...
    	countBox.setFill(Color.TRANSPARENT);
    	...
    	//Creates a BorderPane to place the title of the game '2048' on the left margin, the number of moves StackPane in the center, 
    	//the running score StackPane in the right margin, and the small how to play description label in the bottom.
    	...
    	//Puts the 'titles' BorderPane in the top of this BorderPane and the HBox with the the ALT buttons do at the bottom.  The buttons are center aligned.
    	...
    	//Creates a GridPane of the board.  This is the dark gray background and light gray squares that do not move.
    	...
    	//Creates another GridPane that takes the 16 squares and places their reference into a 4x4 array so that the numbers can be manipulated later on.
    	...
    	//Adding the background grid with the dark gray color and the light gray squares and grid that will have the future tiles that will be moving.
    	...
    	//When the user clicks on the exit button, the final score and move counts, and game over window is displayed.
    	exitButton.setOnAction(e -> {
    		finalScoresScreen();
    	});
    	
    	//When the user clicks on the save button, the current score, move count, and game board will be saved.
    	saveButton.setOnAction(e -> {
    		saveTheCurrentGame();
    	});
        
    	//When the user clicks on the load button, the previous game's score, move count and status of the board is loaded. 
        ...
        //Checks the keyboard for a button click from the user.  Moves UP, DOWN, LEFT, RIGHT, displays the help windows, exits the game, 
    		...
    		switch (e.getCode()) 	
    		{
				case DOWN: 
					...
					{	
						...
						{
							winnerScreen();							//If the user has a 2048 tile, then they will be asked if they would like to continue or quit.
						}
						checkToSeeIfPlayIsAvailable();				//Checks to see if there are any moves available on the board, and if there aren't
																	//the final score and move count will be displayed, along with the game board 5 seconds
				...
				case H:
					if(e.isAltDown())					
						helpingHandScreen();			//If ALT + H is pressed then a window appears telling the user more information 
    	...
    	// Gathers all of the tiles on the array that have a 0.  Generates a new array with X Y coordinates that reference the cells on the board array.
    	// It randomly selects an X Y pair that corresponds to a cell on the board array and places either a 2 or a 4 based on what value Math.random() generates.
    	// 1.) Counts the number of empty squares on the board.
 		// 2.) Generates a list of the empty squares.
 		// 3.) Picks one of the locations on the new array that references the game board.
    ...
    static void fillAnEmptySquare()			
	{
		...
		for (int r = 0; r <= 3; r++)
			for (int c = 0; c <= 3; c++)
				if (board[r][c].getValueOfSquare() == 0) count++;	// 1.) Counts the number of empty squares on the board.
		...
		int cell = (int) (Math.random() * count);					// 3.) Picks one of the locations on the new array that references the game board.
		...
		for (r = 0; r <= 3; r++)									//Based on whether the number is a 2 or a 4, that specific number and its corresponding 
    ...
    //Checks to see if moving down is a valid move.  It checks all of the columns in every row.
	...
	{		
		for (int c = 0; c <= 3; c++)	//Checks all of the columns.
		{
			...
			if (!((row0 == 0 && row1 == 0 && row2 == 0 && row3 == 0)   						||				//If all squares are zeros.
		         ...
		         (row0 == 0 && row1 == 0 && row2 != 0 && row3 != 0 && row2 != row3)				||				//If squares 1 & 2 are zeros and 3 & 4 are not and 3 != 4.
			     (row0 == 0 && row1 != 0 && row2 != 0 && row3 != 0 && row1 != row2 && row2 != row3)	||				//If square 1 is a zero and 2-4 are not and 2 != 3 & 3 != 4.
		         (row0 != 0 && row1 != 0 && row2 != 0 && row3 != 0 && row0 != row1 && row1 != row2 && row2 != row3)))	//If all squares are not zero and 1 != 2 & 2 != 3 & 3 != 4.	
    ...
    //The following method shifts all of the numbers down so that there are no zeros in between all of the non zeros after which it combine numbers
    ...
    //																 4						4
    //
    //After they have been shifted down, the color and number is displayed and the move counter is increased by one.
    
    static void makeMovement()
	{
		for (int c = 0; c <= 3; c++)
		{		
			if ((boardRotated[0][c].getValueOfSquare() + boardRotated[1][c].getValueOfSquare() + boardRotated[2][c].getValueOfSquare() + boardRotated[3][c].getValueOfSquare()) != 0)			// Checks to see if  the columns are != 0.
			{
				if (boardRotated[3][c].getValueOfSquare() == 0)						// If boardRotated[3][c] == 0 shift all the numbers down.
				{
			...
			if ((boardRotated[0][c].getValueOfSquare() + boardRotated[1][c].getValueOfSquare() + boardRotated[2][c].getValueOfSquare() + boardRotated[3][c].getValueOfSquare()) != 0)			// Checks to see if  the columns are != 0.
			{
				if (boardRotated[2][c].getValueOfSquare() == 0)						// If boardRotated[2][c] == 0 shift all the numbers down.
				{
					boardRotated[2][c].setValueOfSquare(boardRotated[1][c].getValueOfSquare()); 
					boardRotated[1][c].setValueOfSquare(boardRotated[0][c].getValueOfSquare());
					boardRotated[0][c].setValueOfSquare(0);
			...
			if ((boardRotated[0][c].getValueOfSquare() + boardRotated[1][c].getValueOfSquare() + boardRotated[2][c].getValueOfSquare() + boardRotated[3][c].getValueOfSquare()) != 0)			// Checks to see if  the columns are != 0.
			{
				if (boardRotated[1][c].getValueOfSquare() == 0)						// If boardRotated[1][c] == 0 shift all the numbers down.
				{
					boardRotated[1][c].setValueOfSquare(boardRotated[0][c].getValueOfSquare());
					boardRotated[0][c].setValueOfSquare(0);
				}
			...
			if (boardRotated[3][c].getValueOfSquare() == boardRotated[2][c].getValueOfSquare())	// If boardRotated[3][c] == boardRotated[2][c] then take what's in boardRotated[2][c] and add it in boardRotated[3][c] 
			{
				...
				scoreCounter[0] += (boardRotated[3][c].getValueOfSquare());				// Adds the newly combined numbers into a running sum.
			}
			
			if (boardRotated[2][c].getValueOfSquare() == boardRotated[1][c].getValueOfSquare())	// If boardRotated[2][c] == boardRotated[1][c] then take what's in boardRotated[1][c] and add it in boardRotated[2][c].
			{
				boardRotated[2][c].setValueOfSquare(boardRotated[1][c].getValueOfSquare() * 2);
				boardRotated[1][c].setValueOfSquare(boardRotated[0][c].getValueOfSquare());
				boardRotated[0][c].setValueOfSquare(0);
				
				scoreCounter[0] += (boardRotated[2][c].getValueOfSquare());				// Adds the newly combined numbers into a running sum.
			}
			
			if (boardRotated[1][c].getValueOfSquare() == boardRotated[0][c].getValueOfSquare())	// If boardRotated[1][c] == boardRotated[0][c] then take what's in boardRotated[0][c] and add it in boardRotated[1][c].
			{
				boardRotated[1][c].setValueOfSquare(boardRotated[0][c].getValueOfSquare() * 2);
				boardRotated[0][c].setValueOfSquare(0);
				scoreCounter[0] += (boardRotated[1][c].getValueOfSquare());				// Adds the newly combined numbers into a running sum.
			}
    ...
    //The first for loop creates backup arrays with all of the values from the moves on the board. The second for loop gets the values of the current board
    //and puts it in the top layer of the 10 undos.  The last two for loops back up all of the scores an moves.
    ...
    //The first if statement checks to make sure that the user has made at least one move before they can undo the board. The first two for loops take the values that are in 
    //the highest layer of the backUpBoard and place them on the board for the user to see. The next three for loops move all of the backUpBoards from a higher slot
    //to a lower one. Ex: backUpBoard 9 is now in 8, 8 is in 7, 7 is in 6 etc.  The following two for loops undo the score and number of moves. 
    ...
    //The only thing that defines the game is the integers at their location and the keepPlayingTheGame boolean, therefore, the only things we need to 
    //save are the integers from the board, score, move count, and the backUp board and the boolean that holds once a 2048 tile is created.
    
    static void saveTheCurrentGame()
    {
    	try(ObjectOutputStream outputTheGame = new ObjectOutputStream(new FileOutputStream("2048.dat")))
    	{
    		for (int scoreCounterVariable = 0; scoreCounterVariable <= 10; scoreCounterVariable++)	//Saves all of the scores to the file.
    			outputTheGame.writeInt(scoreCounter[scoreCounterVariable]);
    		
    		for (int moveCounterVariable = 0; moveCounterVariable <= 10; moveCounterVariable++)		//Saves all of the moves to the file.
    ...
    //Loads the last game that was saved by the user. The data is read in order, so since the current board was read in first, the current board must 
    //be loaded first.
    ...
    static void loadTheLastGame()
    {
    	...
    	{
    		for (int scoreCounterVariable = 0; scoreCounterVariable <= 10; scoreCounterVariable++)	//Loads the scores back into the game.
    			scoreCounter[scoreCounterVariable] = inputTheGame.readInt();
    		
    		for (int moveCounterVariable = 0; moveCounterVariable <= 10; moveCounterVariable++)		//Loads the moves back into the game.
    			moveCounter[moveCounterVariable] = inputTheGame.readInt();
    		
    		
    		for (int row = 0; row <= 3; row++)
    			for (int column = 0; column <= 3; column++)
    				board[row][column].setValueOfSquare(inputTheGame.readInt());							//Sets the current board to the last saved board.
    		...
    		keepPlayingTheGame = inputTheGame.readBoolean();		//Assigns the previous value of keepPlayingTheGame to the current game's value
	...
	//Checks to see if the board has a 2048.  If it does, it returns true for having one.
    static boolean isWinner()							
	{
		for (int row = 0; row <= 3; row++)
			for (int column = 0; column <= 3; column++)
				if (board[row][column].getValueOfSquare() == 2048)  return true;
		return false;
	} // End of isWinner.
   
    //Checks to see if there are any possible moves on the board. If there are possible moves, it skips over finalScoresScreen(). If not, 
    ...
    static void checkToSeeIfPlayIsAvailable()
    {
    	...
    	noRotation();
    	if (isMovementPossible())	movementsPossible++;
    	...
    	if (movementsPossible == 0)    finalScoresScreen();
    }
    
    //Creates a window when the user presses ALT + H or clicks on the help button.  This method provides information on how to play the game 2048.
    //Displays the helpingHand window if the user needs help on how to play the game, they can press ALT + H and it will open this new window.
    
    public void helpingHandScreen()
    {
    	Stage helpAndInstructions = new Stage();
    	
    	Label help = new Label("Welcome to the game 2048.  The objective of this game is to merge the squares of the same\nvalue to get a tile that reads "
    ...
    //The finalScoresScreen() displays the final score the total number of moves that the player has made.
    //If the player does not want to continue playing once they get a 2048 tile, then this window will pop up and tell them there final score and
    ...
    static void finalScoresScreen()
    {
    	...
    	thanksPane.setPadding(new Insets(20,20,20,20));
	 	...
	 	Timeline finalScoresScreen = new Timeline(new KeyFrame(Duration.millis(5000)));	//The finalScoresScreen stays open for 5 seconds.
	 	finalScoresScreen.setOnFinished(closeFinalScoresWindow);
	 	finalScoresScreen.play();
	 	
    }//End of gameOverScreen.
    
    //The gameOverScreen() displays when the user quits or when there are no other possible moves.  A window pops up telling the user 
    //that the game is over and thanks for playing.  When the user doesn't  want to continue the game after getting a 2048 tile, then 
    ...
    static void gameOverScreen()
    {
		...
		Timeline gameOverTimeline = new Timeline(new KeyFrame(Duration.millis(2000), closeGameOver)); //The gameOverTimeline is displayed for 2 seconds.
		gameOverTimeline.play();
		
    }

    //If the user has created a 2048 tile, then this window will pop up asking them if they would like to continue.
    
    static void winnerScreen()
    {
		if(!keepPlayingTheGame)
		{
			keepPlayingTheGame = true;		//Sets this variable to true so that this window will not pop up again.  It is only used for the first 2048 that pops up.
